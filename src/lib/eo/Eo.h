...
#define EO3_DEFINE_INTERFACE_FUNCTION_NORMAL_FUNCTION(NAME, IMPL, ...)    \
  ...
  {                                                             \
    ...
    return func(call.obj_id, call.data EO_PREPROCESSOR_COMMA_IF(EO3_FUNCTION_ENUM_PARAMS_SIZE(__VA_ARGS__)) \
...
#define EO3_DEFINE_CLASS_1(PRIVATE_TYPE, PARENTS, CLASS_NAME, ...)      \
  ...
  {                                                                     \
    ...
    eina_lock_release(&_eo_class_creation_lock);                        \
    _my_class = eo_class_new(&class_desc, EO3_REM PARENTS, NULL);       \
    eina_lock_release(&_my_lock);                                       \
                                                                        \
    eina_lock_take(&_eo_class_creation_lock);                           \
    eina_lock_free(&_my_lock);                                          \
...
#define EO3_DEFINE_CLASS(CLASS, PARENTS, PRIVATE_TYPE)  \
  EO3_DEFINE_CLASS_1(PRIVATE_TYPE, PARENTS, CLASS)
